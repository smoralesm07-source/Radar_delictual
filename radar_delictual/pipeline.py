...
def run(offline: bool = False) -> dict:
    ...
    status_doc={"generated_at":datetime.now(timezone.utc).isoformat(),"offline":offline,"target_period":[TARGET_START_YEAR,TARGET_END_YEAR],"version":"0.3.0","sources":source_status,"coverage":{"mp_region_years":sorted({r["year"] for r in mp_metrics if r.get("territory_level")=="region"}),"cead_annual_2024_drug_communes":annual_drug_communes,"cead_latest_quarantined_records":quarantined,"cead_commune_priority_records":len(commune_priority),"homicide_aml_weight":0.0}}
