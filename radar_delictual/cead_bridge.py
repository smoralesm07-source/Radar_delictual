...
# La comuna Antártica (CUT 12202) no aparece en el diccionario/dataset puente observado.
# Se incorpora al universo esperado, nunca como registro con valor cero.
TERRITORY_SUPPLEMENTS = [{"commune_code":"12202","commune_name":"Antártica","region_code":"12","region_name":"Magallanes y de la Antártica Chilena","data_status":"no_disponible","reason":"Sin observación en la réplica CEAD; no imputar cero."}]
...
def fetch_upstream_snapshot(timeout:int=60) -> tuple[bytes,dict]:
    meta={"repo":UPSTREAM_REPO,"path":UPSTREAM_PATH,"license":LICENSE}; download_url=RAW_URL
    try:
        m=requests.get(CONTENT_API,headers={"Accept":"application/vnd.github+json","User-Agent":"RadarDelictual/0.4"},timeout=20)
        if m.ok:
            doc=m.json(); download_url=doc.get("download_url") or RAW_URL; meta.update({"upstream_blob_sha":doc.get("sha"),"upstream_size":doc.get("size")})
    except Exception: pass
    r=requests.get(download_url,headers={"User-Agent":"RadarDelictual/0.4"},timeout=timeout); r.raise_for_status(); content=r.content
...
def normalize_bridge_frame(frame:pd.DataFrame, start_year:int=2020) -> tuple[pd.DataFrame,dict]:
    ...
    observed_codes=set(df["commune_code"].dropna().astype(str)); missing_expected=[x for x in TERRITORY_SUPPLEMENTS if x["commune_code"] not in observed_codes]
    stats={"min_date":df["fecha"].min().date().isoformat() if len(df) else None,"max_date":df["fecha"].max().date().isoformat() if len(df) else None,"communes":int(df["commune_code"].nunique()),"expected_communes":EXPECTED_COMMUNES,"missing_expected_communes":missing_expected,"offenses":int(df["offense"].nunique()),"monthly_rows":int(len(df)),"years":sorted(df["year"].unique().tolist()) if len(df) else []}
    return df,stats
...
def annualize_bridge(df:pd.DataFrame) -> list[dict]:
    ...
    for r in annual.to_dict("records"):
        out.append({"year":int(r["year"]),"period":str(int(r["year"])),"territory_level":"commune","territory_id":f"CL-{r['commune_code']}","commune_code":r["commune_code"],"commune_name":r["commune_name"],"region_code":r["region_code"],"region_name":r["region_name"],"crime_category":r["offense"],"crime_category_norm":r["offense_norm"],"metric":"casos_policiales","value":int(round(float(r["delito_n"]))),"observation_unit":"caso_policial","source_id":"cead_community_bridge","ultimate_source_id":"cead_estadisticas_delictuales","source_tier":"mirror_of_primary","quality_status":"usable_bridge","acquisition_method":"processed_mirror_of_direct_cead_scrape"})
    return out


def commune_dictionary(df:pd.DataFrame) -> list[dict]:
    cols=["commune_code","commune_name","region_code","region_name"]; d=df[cols].drop_duplicates().sort_values(["region_code","commune_code"]); rows=d.to_dict("records"); present={r["commune_code"] for r in rows}
    rows.extend([dict(x) for x in TERRITORY_SUPPLEMENTS if x["commune_code"] not in present])
    return sorted(rows,key=lambda r:(r["region_code"],r["commune_code"]))


def collect_cead_bridge(start_year:int=2020) -> tuple[list[dict],list[dict],dict,list[dict]]:
    content,meta=fetch_upstream_snapshot(); frame=pd.read_parquet(io.BytesIO(content),engine="pyarrow"); df,stats=normalize_bridge_frame(frame,start_year=start_year)
    valid=stats["communes"]>=340 and bool(stats["max_date"]) and stats["max_date"]>="2025-12-01"; meta.update(stats)
...
def drug_family_rows(records:list[dict]) -> list[dict]:
    aliases={"delitos asociados a drogas","crimenes y simples delitos ley de drogas"}; return [r for r in records if _norm(r.get("crime_category","")) in aliases]
